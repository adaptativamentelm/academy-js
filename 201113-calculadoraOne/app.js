...
const getValues = () =>{
  ...
  for (let x = 0; x < elements.length; x++) {
    if (elements[x].value !== 'potency' && elements[x].value !== 'root' && elements[x].value !== 'result' && elements[x].value !== 'ac' && elements[x].value !== 'symbol') {
      elements[x].addEventListener('click', addNumber.bind(null,elements[x].value));
      console.log('numbers',  elements[x].value);
    }else if (elements[x].value == 'symbol'){
      elements[x].addEventListener('click', addNumber.bind(null,elements[x].value));
    }else{
...
const resultFunction = () => {
  for (let n = 0; n < arrayResult.length; n++) {
    if (arrayResult[n] == '+') {
      total = (arrayResult[n-1]+arrayResult[n+1]);
      inputC.value = total;
      arrayResult = [total];
...
// hacer una uncion que cambie el signo del numero una vez que es presionado
